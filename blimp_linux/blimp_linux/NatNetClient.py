...
class NatNetClient:
    def __init__( self, serverIP, localIP ):
    ...
    def __dataThreadFunction( self, socket ):
        while True:
            ...
            try:
                data, addr = socket.recvfrom( 65565 ) # 32k byte buffer size
                if( len( data ) > 0 ):
                    # print(data)
    ...
    def run( self ):
        ...
        # self.commandThread = Thread( target = self.__dataThreadFunction, args = (self.commandSocket, ), daemon=True )
    ...
    def shutdown(self):
        """
        Gracefully stop data and command threads and close sockets.
        """
        print("Shutting down NatNetClient...")

        # Signal threads to stop
        # self._stop_event.set()

        # Close sockets (this unblocks recvfrom() calls)
        if self.dataSocket:
            try:
                self.dataSocket.close()
                print("Data socket closed.")
            except Exception as e:
                print("Error closing data socket:", e)
        ...
        if self.commandSocket:
            try:
                ...
                print("Error closing command socket:", e)

        # Wait for threads to exit
        if self.dataThread and self.dataThread.is_alive():
            self.dataThread.join(timeout=1.0)
            print("Data thread stopped.")

        if self.commandThread and self.commandThread.is_alive():
            self.commandThread.join(timeout=1.0)
            print("Command thread stopped.")

        print("NatNetClient fully shut down.")
